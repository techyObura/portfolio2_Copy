...
const UpdateProject = ({ menu }) => {
  const { projectId } = useParams();
  const navigate = useNavigate();
  const { currentUser } = useSelector((state) => state.user);
  const getProjectApi = `${api}/projects/getProjects?projectId=${projectId}`;
  const [file, setFile] = useState(null);
  const [imageUploadProgress, setImageUploadProgress] = useState(null);
  const [imageUploadError, setImageUploadError] = useState(null);
  const [formData, setFormData] = useState({});
  const [publishError, setPublishError] = useState(null);
  const update_project_api = `${api}/projects/edit/${formData._id}`;

  useEffect(() => {
    const fetchPost = async () => {
      try {
        const res = await fetch(getProjectApi);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(update_project_api, {
        method: "PUT",
        headers: {
